...
fn main() {
    ...
    for (i, j) in distances.iter().map(|(pair, _)| pair).take(JUNCTIONS) {
    ...
    let mut sizes: Vec<_> = groups.iter().map(|g| g.len()).collect();
    sizes.sort_unstable();
    let total: usize = sizes.iter().rev().take(3).product();
